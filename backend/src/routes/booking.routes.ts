...
import { paymentUpload, uploadPaymentScreenshot, getPaymentScreenshotUrl } from "../middleware/uploadPayment";
...
router.patch("/:id/payment-status", requireAdminDevice, async (req, res) => {
  try {
    ...
    if (!allowed.includes(paymentStatus)) return res.status(400).json({ success: false, message: "Invalid payment status" });
    const updated = await prisma.booking.update({ where: { id }, data: { paymentStatus } });
    res.json({ success: true, booking: updated });
  } catch (error) {
    console.error(error);
...
router.get("/:id/screenshot", requireAdminDevice, async (req, res) => {
  try {
    const booking = await prisma.booking.findUnique({ where: { id: getParam(req.params.id) }, select: { paymentScreenshot: true } });
    if (!booking) return res.status(404).json({ success: false, message: "Booking not found" });
    if (!booking.paymentScreenshot) return res.status(404).json({ success: false, message: "No screenshot on file" });
...
router.post("/", paymentUpload.single("paymentScreenshot"), async (req, res) => {
  try {
    ...
    // totalAmount stays 0 if no RoutePricing row exists — never fall back
    // to vehicle.price for taxi bookings, price must be confirmed via route pricing only.
