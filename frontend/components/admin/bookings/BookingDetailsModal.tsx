...
export default function BookingDetailsModal({
  ...
  return (
    <>
      ...
      <div
        className="
          fixed z-50 bg-[#141414] text-white overflow-y-auto

          /* Mobile: full-screen sheet from bottom */
          inset-x-0 bottom-0 top-[5vh] rounded-t-3xl

          /* Desktop: centered modal */
          md:inset-0 md:m-auto md:rounded-2xl md:max-w-2xl md:max-h-[90vh]
          md:border md:border-[#252525] md:top-auto md:bottom-auto
          md:h-fit
        "
      >
        ...
        {/* Header */}
        <div className="flex items-start justify-between px-5 pt-4 pb-3 border-b border-[#1c1c1c] sticky top-0 bg-[#141414] z-10">
          <div>
            <h2 className="text-lg font-bold md:text-2xl">Booking Details</h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs text-[#8a8a8a] capitalize">{booking.serviceType}</span>
              <span
          ...
          <button
            onClick={onClose}
            className="rounded-xl p-2 hover:bg-[#252525] active:bg-[#2a2a2a]"
          >
            <X size={20} />
          </button>
        </div>
        ...
        {/* Content */}
        <div className="px-4 py-4 space-y-3 md:px-6 md:py-5 md:space-y-4 pb-8">

          {/* CUSTOMER — prominent on mobile with tap-to-call */}
          <Section title="Customer">
            <p className="text-base font-semibold text-white">{booking.customer?.name}</p>
          ...
          {booking.airport && (
            <Section title="Airport Transfer">
              ...
              <Detail
                label="Travel Date"
                value={new Date(booking.airport.travelDate).toLocaleDateString("en-IN")}
              />
          ...
          {booking.selfDrive && (
            <Section title="Self Drive">
              ...
              <Detail
                label="Pickup Date"
                value={new Date(booking.selfDrive.pickupDate).toLocaleDateString("en-IN")}
              />
              <Detail
                label="Return Date"
                value={new Date(booking.selfDrive.returnDate).toLocaleDateString("en-IN")}
              />
          ...
          {booking.luxury && (
            <Section title="Luxury Car">
              ...
              <Detail
                label="Event Date"
                value={new Date(booking.luxury.eventDate).toLocaleDateString("en-IN")}
              />
          ...
          {booking.wedding && (
            <Section title="Wedding Car">
              ...
              <Detail
                label="Wedding Date"
                value={new Date(booking.wedding.weddingDate).toLocaleDateString("en-IN")}
              />
          ...
          {booking.tempo && (
            <Section title="Tempo / Urbania">
              ...
              <Detail
                label="Travel Date"
                value={new Date(booking.tempo.travelDate).toLocaleDateString("en-IN")}
              />
