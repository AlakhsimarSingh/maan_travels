    ...
    <section className="py-24 bg-[#0b0b0b]">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <Reveal className="text-center mb-14">
          <p className="uppercase tracking-[0.3em] text-[#ecb100]">Why Choose Us</p>
          <h2 className="mt-4 text-4xl font-bold text-white">Luxury Travel Beyond Ordinary</h2>
          <p className="mt-5 mx-auto max-w-2xl text-[#8a8a8a]">
            Experience premium mobility with elegant vehicles, professional service and unmatched comfort.
          </p>
        </Reveal>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {reasons.map((item, i) => (
            <Reveal key={item.title} delay={i * 70}>
              <div className="rounded-3xl border border-[#252525] bg-[#141414] p-7 transition duration-300 hover:border-[#ecb100] hover:-translate-y-1">
                <div className="text-[#ecb100]">{item.icon}</div>
                <h3 className="mt-5 text-xl font-semibold text-white">{item.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-[#8a8a8a]">{item.description}</p>
              </div>
            </Reveal>
          ))}
        </div>
