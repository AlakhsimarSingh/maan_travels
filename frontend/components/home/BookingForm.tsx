...
export default function BookingForm({ vehicles }: { vehicles: Vehicle[] }) {
  const [rideMode, setRideMode] = useState<"oneway" | "round" | "local">("oneway");
  ...
  const closeCategory = () => {
    hoverTimeout.current = setTimeout(() => setActiveCategory(null), 140);
  };

  return (
    <div id="book" className="mx-auto max-w-5xl rounded-3xl border border-[#252525] bg-[#141414] p-8">

      <h2 className="mb-8 text-3xl font-bold text-white">Book Your Journey</h2>
          ...
          <div className="space-y-6">
              ...
              {(Object.keys(grouped) as Category[]).map(cat => (
                <div key={cat} className="relative">
                  <div
                    ...
                    className="bg-[#111] border border-[#252525] rounded-xl p-4 cursor-pointer transition-all duration-200 hover:border-[#ecb100]/40"
                  >
                    <p className="text-white font-medium">{cat}</p>

                    {selectedVehicle?.category?.toLowerCase().includes(cat.toLowerCase()) && (
                      <p className="text-xs text-[#ecb100] mt-1">Selected: {selectedVehicle.name}</p>
                    )}
                  </div>
                  ...
                  {activeCategory === cat && (
                    <div
                      onMouseEnter={() => openCategory(cat)}
                      onMouseLeave={closeCategory}
                      className="absolute left-0 top-full mt-2 w-full bg-black/95 border border-[#252525] rounded-xl z-50 overflow-hidden"
                    >
                      {grouped[cat].map(v => (
                        <div
                          key={v.id}
                          onClick={() => { setSelectedVehicle(v); setActiveCategory(null); }}
                          className={`
                            px-4 py-3 text-sm cursor-pointer transition-colors
                            ${selectedVehicle?.id === v.id ? "bg-[#1f1f1f] text-[#ecb100]" : "text-white hover:bg-white/5"}
                          `}
                        >
                          {v.name}
                        </div>
            ...
            {selectedVehicle && (
              <div className="rounded-xl border border-[#252525] bg-black/40 p-4 text-sm text-white">
                ...
                <p className="text-[#ecb100] mt-1">Selected: {selectedVehicle.name}</p>
              </div>
            )}

            <TaxiBookingForm rideMode={rideMode} selectedVehicleId={selectedVehicle?.id || ""} />
          </div>
        </TabsContent>
